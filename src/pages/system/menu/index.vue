    ...
    >
      <coadmin-form
        ...
        class="q-pa-md row q-col-gutter-x-xl q-col-gutter-y-md">
          <coadmin-tree-select class="col-12 col-sm-6" form-label="上级菜单" :disable="!!crud.status.view"
            :nodes="menuDatas"
          ...
          <coadmin-field class="col-12 col-sm-6" form-label="ID" readonly>
            <template v-slot:control>{{form.id}}</template>
          </coadmin-field>
          <coadmin-option-group class="col-12 col-sm-6" form-label="菜单类型" v-model="form.type" inline
          ...
          />
          <coadmin-input class="col-12 col-sm-6" form-label="排序" v-model.number="form.sort" type="number" :disable="!!crud.status.view" :rules="[
              val => (!!val) || '必填'
              ]"/>
          <coadmin-input class="col-12 col-sm-6" form-label="图标" v-model="form.icon" key="icon"
          ...
          <coadmin-input class="col-12 col-sm-6" form-label="菜单名称" v-model="form.title" key="title"
            :disable="!!crud.status.view"
            :rules="[
              val => (!!val) || '必填'
              ]"/>
          <coadmin-option-group class="col-12 col-md-4" form-label="外链菜单" v-model="form.iframe" inline
            v-if="form.type===0 || form.type===1"
            :disable="!!crud.status.view"
            :options="[
              {label: '是', value: true},
            ...
            />
          <coadmin-option-group class="col-12 col-md-4" form-label="菜单缓存" v-model="form.cache" inline
            v-if="form.type===0 || form.type===1"
            :disable="!!crud.status.view"
            :options="[
              {label: '是', value: true},
            ...
            />
          <coadmin-option-group class="col-12 col-md-4" form-label="菜单可见" v-model="form.hidden" inline
            v-if="form.type===0 || form.type===1"
            :disable="!!crud.status.view"
            :options="[
              {label: '是', value: false},
