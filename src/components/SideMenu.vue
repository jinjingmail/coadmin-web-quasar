    ...
    item () {
      ...
      if (ri.children.length === 1 && !ri.alwaysShow) {
        ...
        if (isExternal(child.path)) {
          return { path: child.path, name: child.name, title: m.title, caption: m.caption, icon: m.icon, iconColor: meta.iconColor }
        }
        return { path: path.resolve(this.resolvePath(ri.path), child.path), name: child.name, title: m.title, caption: m.caption, icon: m.icon, iconColor: meta.iconColor }
      } else {
